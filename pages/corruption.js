...
function renderCorruption() {
  ...
  return `
    <section class="chapter" id="corruption">
      ...
      <p>Aevum healthy is light and time and cycles. Aevum corrupted is light that never stops, time that never moves, cycles that never complete. The same thing, but somewhat wrong.</p>
      ...
      ${tc('Path One: Overexposure', 'Most Common', `
        ...
        <br>
        <p>This is what weapon side effects are building toward if left unaddressed, to Integrated people who only use one crystal type, to the Sundered One over centuries of imprisonment with no rest, no cycle nor counterbalance.</p>
        <br>
        <p>An Aevum-corrupted person isn't someone who loves light and time. They're someone who can no longer experience darkness, rest or completion.</p>
        <br>
        <p>Casual use usually shows no corruption risk. Side effects are possible, but also reversible. Regular use over months has persistent side effects. In sustained heavy use over years without counterbalance, corruption becomes a real risk. <br> Total saturation is corruption; at this stage it's about which expression dominates, healthy or corrupted, and the person has decreasing influence over that.</p>
      `)}
      ...
      ${tc('Path Three: Severance', 'Most Violent', `
        ...
        <br>
        <p>Every category exists in relation to the others. Aevum needs Limen: light needs darkness, cycles need endings. Profundis needs Vertex: memory needs change, depth needs disruption. Humus needs Anima: body needs consciousness. <br> They keep each other healthy instead of cancelling eachother.</p>
        <br>
        <p>When a category is severed from its natural counterbalances, it corrupts faster than any other path. This is what the Cores do.</p>
        <p>The mini-star at the top of the Spire is beautiful and it is also the single largest point source of corruption in Solaria, constantly producing the Aevum corrupted expression. <br> The city breathes that in every day and calls it warmth.</p>
        <p>Full severance produces constant corrupted output, not recoverable while the severance continues. <br> Severance of multiple beings simultaneously is what the Perfect Machine attempts.</p>
      `)}
      ...
      ${tc('In Objects', '', `
        <p>Crystal that's been exposed to corrupted expression absorbs it slowly. <br> Weapons used primarily for violation absorb that corruption into their structure. Old weapons used for the wrong purposes long enough start expressing corruption passively.</p>
        <p>The Cores are maximally corrupted objects. <br> The Spire's architecture is partly designed to contain this. Everything above the Undercroft is slightly Aevum-corrupted and has been for so long that residents don't have a baseline for comparison.</p>
      `)}
      ${tc('In Humans', '', `
      ...
      `)}
      ${tc('In Gods', '', `
        <p>In gods corruption is expressed differently because gods ARE their category. <br> What happens instead is that the corrupted expression becomes their dominant mode. Their power doesn't diminish but its nature changes. <br> They become harder to work with, harder to approach, harder to reason with. Less of…themselves.</p>
      `)}
      ${tc('In the Integrated', '', `
        <p>In the Integrated, corruption is the most interesting because they're already hybrid. <br> Sometimes the human part corrupts and the divine part remains healthy. Sometimes the divine part corrupts and the human part pushes back. Sometimes they corrupt in opposite directions and the person exists in a state of internal tension that is exhausting and also the thing keeping them functional. <br> The most stable people are the ones whose two natures are corrupting in ways that counterbalance each other.</p>
      `)}
      </div>
